...
** DONE Run Extension Cord
   CLOSED: [2012-10-05 Fri 11:51]

** DONE Complete the webdriver review comments
   CLOSED: [2012-10-05 Fri 12:45]
** TODO Architect extension cord
   Current Issues:
   Unable to report based on phase
   Currently, Quality Center is organized into folders and each team has a varying folder depth. 
   Structure of the folders
   Use a tag based structure for organizing test cases
   First Issue
   Each test case gets tagged depending on its path
   ...
   1) It belongs to a product and a category
   2) It belongs to a release and phase under the release
  ...
  For each release, each test case is repeated for each phase

1) Attach django-taggit to the testplan-testcase model - DONE - is a decent way to proceed
2) Show this in the test plan page - This can be tricky.

   a) The issue is that currently editing the details of a test plan and adding test cases to a test plan are at the same places. Split it into two pages.
   Edit TestPlan Details
   Add TestCases
   
   This will mean that in the TestPlan, there will be two columns. one is to add testcases and the other is to edit testplan details

   There will be two pages for each of these actions
   
   Once the user has created a test plan, he can add test cases to the test plan
